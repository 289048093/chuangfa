...
public abstract class BaseAction<T extends BaseEntity> extends ActionSupport {
     ...
     * 
     */
    private static final long serialVersionUID = -9009554355791289160L;
    /**
     * 成功的标识
     */
    ...
    public BaseAction(){
        try {
            ...
            int start = StringUtil.isBlank(getRequest().getParameter("pager.offset"))
                    || Integer.parseInt(getRequest().getParameter("pager.offset")) < 0 ? cloudContext.getPageInfo()
                    .getStart() : Integer.parseInt(getRequest().getParameter("pager.offset"));
